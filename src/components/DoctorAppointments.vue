...
const fetchAppointments = async (id) => {
  try {
    const response = await fetch(`${baseUrl}/api/appointments/doctor/${id}`, {
      method: 'GET',
      headers: {
...
const startAppointment = async (appointmentId) => {
  const confirmStart = confirm('Вы уверены, что хотите начать прием?');
  if (!confirmStart) return;

  try {
    const response = await fetch(`${baseUrl}/api/appointments/start/${appointmentId}`, {
      method: 'PATCH',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json',
    ...
    const updatedAppointments = appointments.value.map(appointment => {
      if (appointment.id === appointmentId) {
        return { ...appointment, status: 'В процессе' }; // Обновите статус, если необходимо
      }
      return appointment;
    });
    ...
    console.error('Не удалось начать прием:', error);
    alert('Ошибка при начале приема: ' + error.message);
  }
};

// Функция для отмены приема
const cancelAppointment = async (appointmentId) => {
  const confirmCancel = confirm('Вы уверены, что хотите отменить прием?');
  if (!confirmCancel) return;

  try {
    const response = await fetch(`${baseUrl}/api/appointments/reject/${appointmentId}`, {
      method: 'PATCH',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json',
    ...
    // Обновление состояния для локальной записи, если прием успешно отменен
    const updatedAppointments = appointments.value.map(appointment => {
      if (appointment.id === appointmentId) {
        return { ...appointment, status: 'Отменён' }; // Обновите статус
      }
      return appointment;
    });
    appointments.value = updatedAppointments;
    ...
    console.error('Не удалось отменить прием:', error);
    alert('Ошибка при отмене приема: ' + error.message);
  }
};
...
<template>
  <div class="spec">
    ...
    <button class="tabs2" @click="goApps()" style="color: black">Общая информация</button>
    <button class="tabs">Мои приёмы</button>
      ...
      <ul>
        <li v-for="appointment in filteredAppointments" :key="appointment.id" class="appointment-item">
          <div class="appointment-header" @click="toggleAppointment(appointment)">
            ...
            <button v-if="appointment.status === 'В ожидании' && role === 'Врач'" @click.stop="startAppointment(appointment.id)" class="btn-start">
              Начать прием
            </button>
            <button v-if="appointment.status === 'В ожидании' && role === 'Администратор'" @click.stop="cancelAppointment(appointment.id)" class="btn-start"> <!-- Изменено здесь -->
              Отменить
            </button>
            <button v-else @click.stop="goAppInfo(appointment.id)" class="btn-start">
              Подробности
            </button>
          </div>
          <div class="appointment-details" :class="{ 'open': openedAppointment === appointment }">
            <p>Пациент: {{ appointment.patient_full_name }}</p>
